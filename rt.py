...
def route():
    ...
    def samplesToRoute(samples): # samples must be an (nStations, 2) array
        ...
        while True:
            for i in range(len(routes)):
                ...
                idx = (int(len(candList) * samples[count]) + i << 4) % (nSelThis + 1)
                if idx == nSelThis:
    ...
    def routeCost(routes):
        ...
        for rt in routes:
            ...
            if rtTraf == 0:
                rtTraf = 1e-20 # this route should be abandoned
            cost += kDist * rtDist + kTraf / rtTraf + kPop / rtPop
        return cost

    mtr = Metropolis(0.5, 0.1)
    val, samples = mtr.anneal(lambda sp: routeCost(samplesToRoute(sp)), nStations, 0.9, 0.5, 0.1, 10)

    routes = samplesToRoute(samples)
    writer = pd.ExcelWriter("data/" + str(nRoutes) + "routes.xlsx")
    for i in range(len(routes)):
        rt = routes[i]
